...
:width: 35%
:align: center
:alt:  A simple circuit, showing a \SI{9
 A simple circuit, showing a \SI{9
"`{V} battery and a 2 {\rm \Omega} resistor. For ease in analyzing circuits, we suggest drawing a ``battery arrow" above batteries that goes from the negative to the positive terminal.}
...
:width: 40%
:align: center
:alt:  The same circuit as in Figure \ref{fig:circuits:batteryresistor
 The same circuit as in Figure \ref{fig:circuits:batteryresistor
``` showing the two regions over which the electric potential is constant.}
...
:width: 40%
:align: center
:alt:  A fluid dynamics analogue of the circuit in Figure \ref{fig:circuits:batteryresistor
 A fluid dynamics analogue of the circuit in Figure \ref{fig:circuits:batteryresistor
```, where a pump plays the role of the battery, and a narrow pipe that of a resistor.}
:::{note} Example
:label: ex:circuits:tworesistorsTwo resistors, of $2 {\rm \Omega}$ and $4 {\rm \Omega}$, respectively, are connected in series to a $12 {\rm V}$ battery. What is the current through each of the resistors, and what is the voltage across each resistor?}
...
:width: 42%
:align: center
:alt:  The resistors from the circuit in Figure \ref{fig:circuits:tworesistors
 The resistors from the circuit in Figure \ref{fig:circuits:tworesistors
``` have been combined in series to simplify the circuit.}
...
```{math}
\Delta V &= R_{eff}I\\
\therefore I&=\frac{\Delta V}{R_{eff}}=\frac{(12 {\rm V})}{(6 {\rm \ohm})}=2 {\rm A}
```
...
The circuit in [](#fig:circuits:realbatterycircuit) is now identical to that analyzed in [](#ex:circuits:tworesistors), and can be treated the same way. We can combine the $2 {\rm \Omega}$ resistor with the internal resistance, $r$, in series to obtain an effective resistor, $R_{eff}=r+R$. The voltage drop across the effective resistor will be the same as the potential difference across the ideal battery, and we can make use of Ohm's Law to find the internal resistance, $r$:
```{math}
...
:width: 50%
:align: center
:alt: The circuit from Figure \ref{fig:circuits:loop
The circuit from Figure \ref{fig:circuits:loop
``` using the roller coaster analogy.}
...
:width: 60%
:align: center
:alt: The same circuit as in Figure \ref{fig:circuits:bigcircuit
The same circuit as in Figure \ref{fig:circuits:bigcircuit
