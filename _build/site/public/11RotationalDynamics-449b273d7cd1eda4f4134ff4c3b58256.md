...
```{exercise} Opening Thought
:label: Opening Thought
...
A. It will be easier to lift the block if the rock is close to the block.
B. It will be easier to lift the block if the rock is far from the block.
C. It does not matter where he places the rock, as long as the bar is short.
D. It does not matter where he places the rock, as long as the bar is long.
```
```{solution} 
:class: dropdown
A.
...
\item [Section "{Circular Motion}"](#circular-motion) on kinematics for circular motion.
