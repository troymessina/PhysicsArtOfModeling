...
:::{note} Solution
:class: dropdown
This is the same problem we solved in [Example %s](#ex:applyingnewtonslaws:block). In that case, we solved for the acceleration of the block using Newton's Second Law and then used kinematics to find how far the block went. We can solve this problem in a much easier way using conservation of energy. 
