...
Usually, when multiple blocks are stacked on each other, it is easiest to start with the forces on the top block. In this case, the top block is in the same condition as the block from [Example %s](#ex:newtonslaws:blockI). The forces exerted on the top block are:
