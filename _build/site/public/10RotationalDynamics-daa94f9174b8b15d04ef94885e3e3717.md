---
title: Chapter 10 - Rotational dynamics
numbering:
  headings:
  heading_1:
    start: 11
...
* [Section %s](#sec:momentumandcm:circularmotion) on kinematics for circular motion.
...
4. The angular acceleration points into the page and the angular velocity points out of the page.
:::{tip} Answer
:class: dropdown
:label: cp:rotationaldynamics:ant
4.
:::
...
:::{tip} Answer
:label: cp:rotationaldynamics:angvel
:class: dropdown
3.
:::
...
:::{note} Solution
:class: dropdown
:label: ex:rotationaldynamics:rocketsoln
...
:::{note} Solution
:label: ex:rotationaldynamics:dumbbell_2msoln
:class: dropdown
...
3. Let us now calculate the angular acceleration of the dumbbell about an axis that goes through one of the masses, as illustrated in [](#fig:rotationaldynamics:dumbbell_end).
...
:class: dropdown
We take a small mass element $dm$ of the ring, as shown in [](#fig:rotationaldynamics:ring). 
...
:::{note} Solution
:label: ex:rotationaldynamics:rodsoln
:class: dropdown
...
:::{note} Solution
:label: ex:rotationaldynamics:scalesoln
:class: dropdown
...
We know from the [example](#ex:rotationaldynamics:ring), that the infinitesimal moment of inertia, $dI$, of a ring of radius $r$ and infinitesimal mass, $dm$, about its axis of symmetry is given by:
\begin{align*}
dI = dm r^2
\end{align*}
...
* $\vec R$, a force exerted by the hinge on the bar at the end next to the wall[^26]. We expect that the force from the hinge will have both a horizontal component, $R_x$, and a vertical component, $R_y$, in order for the net force on the bar to be zero.
...
:::
[^25]:They will have different linear accelerations, but the angular acceleration (and angular velocity) will be the same for all particles if they are moving in unison.
[^26]:We chose the letter R for "Reaction", as this is the force of reaction from the hinge as the bar pushes against the hinge.