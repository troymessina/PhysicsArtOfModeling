...
:::{tip} Checkpoint:label: cp:chargesfields:conservative
...
We model this by assuming that the electron and proton are point charges a distance of $1 {\rm \angstrom}=1\times 10^{-10} {\rm m}$ apart (1 \r Angstrom is about the size of the hydrogen atom). The proton and electron have the same charge with magnitude $e=1.6\times 10^{-19} {\rm C}$, so the (attractive) electric force between them has a magnitude of:
```{math}
F_e = k\frac{Q_1Q_2}{r^2}=(9\times 10^9 {\rm N\cdot m^2/C^{2}})\frac{(1.6\times 10^{-19} {\rm C})(1.6\times 10^{-19} {\rm C})}{(1\times 10^{-10} {\rm m})^2}=2.3\times 10^{-8} {\rm N}
```
which is a small number, but acting on a very small mass. In comparison, the force of gravity between an electron ($m_e=9.1\ti}mes 10^{-31} {\rm kg}$) and a proton ($m_p=1.7\times 10^{-27} {\rm kg}$) is given by:
```{math}
F_g=G\frac{m_em_p}{r^2}=(6.7\times 10^{-11} {\rm Nm^2/kg^2})\frac{(9.1\times 10^{-31} {\rm kg})(1.7\times 10^{-27} {\rm kg})}{(1\times 10^{-10} {\rm m})^2}=1.04\times 10^{-47} {\rm N}
```
...
&=-(4.5e-5 {\rm N})\hat x-(7.8e-5 {\rm N})\hat y-(9.0e-5 {\rm N})\hat x+(1.6e-4 {\rm N})\hat y\\
&=-(13.5e-5 {\rm N})\hat x+(8.2e-5 {\rm N})\hat y
```
which has a magnitude of $15.8e-5 {\rm N}$.
...
:::{tip} Checkpoint:label: cp:chargesfields:edirection
...
:::{note} Example
:label: ex:chargesfields:epointchargesTwo charges, $Q_1=1 {\rm nC}$, and $Q_2=-2 {\rm nC}$ are held fixed at two corners of an equilateral triangle with sides of length $a=1 {\rm cm}$, with a coordinate system as shown in [](#fig:chargesfields:chargetriangle). What is the electric field vector at the third corner of the triangle? 
...
```{math}
E_1=\left |k\frac{Q_1}{a^2}\right |=(9e9 {\rm N\cdot m^2/C^{2}})\frac{(1e-9 {\rm C})}{(0.01 {\rm m})^2}=9e4 {\rm N/C}
```
and components:
```{math}
\vec E_1&=E_1\cos(60 {\rm \degree})\hat x+E_1\sin(60 {\rm \degree})\hat y\\
&=(4.5e4 {\rm N/C})\hat x+(7.8e4 {\rm N/C})\hat y
```
Similarly, the electric field from $Q_2$ has magnitude:
```{math}
E_2=\left |k\frac{Q_2}{a^2}\right |=(9e9 {\rm N\cdot m^2/C^{2}})\frac{(2e-9 {\rm C})}{(0.01 {\rm m})^2}=1.8e5 {\rm N/C}
```
and components:
```{math}
\vec E_2&=E_2\cos(60 {\rm \degree})\hat x-E_2\sin(60 {\rm \degree})\hat y\\
&=(9.0e4 {\rm N/C})\hat x-(1.6e5 {\rm N/C})\hat y
```
Finally, we can add the two force vectors together to obtain the net force on $q$:
```{math}
\vec E^{net}&=\vec E_1+\vec E_2\\
&=(4.5e4 {\rm N/C})\hat x+(7.8e4 {\rm N/C})\hat y+(9.0e4 {\rm N/C})\hat x-(1.6e5 {\rm N/C})\hat y\\
&=(13.5e4 {\rm N/C})\hat x-(8.2e4 {\rm N/C})\hat y
```
which has a magnitude of $15.8e4 {\rm N/C}$. By knowing the electric field at the empty corner of the triangle, we can now calculate the net electric force that would act on any charge placed in that location. For example, if we place a charge $q=-1 {\rm nC}$ (as in [](#ex:chargesfields:chargetriangle)), we can easily find the corresponding electric force:
```{math}
\vec F_q &= q\vec E=(-1 {\rm nC})\left[ (13.5e4 {\rm N/C})\hat x-(8.2e4 {\rm N/C})\hat y \right]\\
&=-(13.5e-5 {\rm N})\hat x+(8.2e-5 {\rm N})\hat y
```
...
:::{tip} Checkpoint:label: cp:chargesfields:conductorield
...
:::{note} Example
:label: ex:chargesfields:ringA ring of radius $R$ carries a total charge $+Q$. Determine the electric field a distance $a$ from the centre of the ring, along the axis of symmetry of the ring.}
