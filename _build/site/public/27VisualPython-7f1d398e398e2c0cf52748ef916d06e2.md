...
Change the code for the ball example above so that the ball is thrown upward along the $z$-axis. Add the acceleration due to gravity using the kinematic equation.
```{math}
\vec r(t + \Delta t) = \vec r(t) + \vec v\Delta t + \frac{1}{2}\vec a\Delta t^2
```
:::{note} Solution
:class: dropdown
The velocity will now be ```ball.vel = vec(0,0,0.5)```. There is an acceleration ```ball.acc = vec(0,0,-9.8)```. The position update will now be
```python
