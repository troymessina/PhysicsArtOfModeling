...
:align: center
:alt: Plot of position as a function of time using the values from [](#tab:DescribingMotionIn1D:1dmotion).
Plot of position as a function of time using the values from [](#tab:DescribingMotionIn1D:1dmotion).
```
The data plotted in [](#fig:MomentumAndCM:1dxvst) show that the $x$ position of the ball increases linearly with time (i.e. it is a straight line and the position increases at a constant rate). This means that in equal time increments, the rock will cover equal distances. Note that we also had the liberty to choose when we define $t=0$; in this case, we chose that time is zero when the rock is at $x_o=0.5 {\rm m}$. 

:::{tip} Checkpoint
Using the data from [](#tab:DescribingMotionIn1D:1dmotion), at what position along the $x$-axis will the ball be when time is $t=9.5 {\rm s}$, if it continues its motion undisturbed?
1. $5.0 {\rm m}$
2. $5.25 {\rm m}$
3. $5.75 {\rm m}$
4. $6.0 {\rm m}$
...
\Delta x = v\Delta t
```
It is then simple to describe the rock's position as a function of time.
```{math}
:label: eq:momentumandcm:constvelposeq
x(t) = x_o + \Delta x\\
x(t) = x_o + v\Delta t
``` 
As long as the velocity is constant, we can use equation {eq}`eq:momentumandcm:constvelposeq` to determine the position of an object between any two points in time. A graph of this equation ($x(t)$ vs. $t$) is shown in [](#fig:momentumandcm:)
```{figure} ./figures/MomentumAndCM/1dxvst.png
:width: 70%
:label: fig:DescribingMotionIn1D:1dxvst
:align: center
